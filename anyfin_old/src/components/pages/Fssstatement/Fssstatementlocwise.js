...
const Fssstatementlocwise = () => {
    const [Fssstatementlocwise, setFssstatementlocwise] = useState([]); // Initialize as an empty array
    const [FssstatementlocwiseUnload, setFssstatementlocwiseUnload] = useState([]); // Initialize as an empty array
    ...
    const getAllNavigation = () => {
        commonService.getAll(apiUrlsService.getNavigationFSSStatements).then((response) => {
            if (response && response.data) {
    ...
    const getAllFssstatementlocwiseload = () => {
        commonService.getAll(apiUrlsService.getFssStatementsLocwise).then((response) => {
            if (response && response.data) {
                setFssstatementlocwise(response.data);
            }
        }).catch((error) => {
            handleApiError(error);
        });
    ...
    const getAllFssOptions = () => {
        commonService.getAll(apiUrlsService.getAllFssstatementLocWise).then((response) => {
            if (response && response.data) {
                    ...
                    <div className="mb-2">
                        <div className="table-responsive card">
                            <table className="table">
                                ...
                                <tbody>
                                    {Fssstatementlocwise.length > 0 ? (
                                        Fssstatementlocwise.map((item, index) => (
                                            <tr key={item.id}>
                                                <td>{index + 1}</td>
                                                <td>{item.description}</td>
                                                <td>{item.prev_amount}</td>
                                                <td>{item.curr_amount}</td>
                                                <td>{item.diff}</td>
                                                <td>{item.percent_diff}</td>
                                                <td>{item.prev_parent}</td>
                                                <td>{item.curr_parent}</td>
                                                <td>{item.parent_diff}</td>
                                                {/* <td>
...
export default Fssstatementlocwise;
