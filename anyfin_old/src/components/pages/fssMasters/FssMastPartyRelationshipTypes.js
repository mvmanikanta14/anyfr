...
const FssMastPartyRelationshipTypes = () => {
    const [FssMastPartyRelationshipTypes, setFssMastPartyRelationshipTypes] = useState([]);
    const [FssMastPartyRelationshipTypesAdd, setFssMastPartyRelationshipTypesAdd] = useState([]);
    ...
    const deleteFssMastPartyRelationshipTypes = (id) => {
        // Construct the correct API URL with the ID
        const deleteUrl = `${apiUrlsService.deleteFssMastPartyRelationshipTypes}/${id}`;
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    ...
                    getAllFssMastPartyRelationshipTypes(); // Refresh the list
                } else {
    ...
    const getAllFssMastPartyRelationshipTypes = () => {
        let pagedata = {
        };

        commonService.add(apiUrlsService.getFssMastPartyRelationshipTypes, pagedata).then(
            (response) => {
                if (response && response.data) {
                    console.log("mani", response.data)
                    setFssMastPartyRelationshipTypes(response.data);
    ...
    const onSubmit = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsService.addFssMastPartyRelationshipTypes, requestData)
                .then((res) => {
                    setFssMastPartyRelationshipTypesAdd([...FssMastPartyRelationshipTypesAdd, res.data]);
            ...
            commonService.update(`${apiUrlsService.editFssMastPartyRelationshipTypes}/${editData.id}`, requestData)
                .then((response) => {
                    if (response) {
                        const updatedFssMastPartyRelationshipTypes = FssMastPartyRelationshipTypes.map((item) =>
                            item.id === editData.id ? response.data : item
                        );
                        console.log("Updated Data:", updatedFssMastPartyRelationshipTypes);
                        setFssMastPartyRelationshipTypesAdd(updatedFssMastPartyRelationshipTypes);
    ...
    const handleShowEdit = (id) => {
        const itemToEdit = FssMastPartyRelationshipTypes.find((item) => item.id === id)
    ...
    const activeShares = FssMastPartyRelationshipTypes.filter(item => item.is_active);
    const inactiveShares = FssMastPartyRelationshipTypes.filter(item => !item.is_active);
        ...
        <div className="">
            <div className="">
                <div className="bread_crumb">
                    <div className=" ">
                        <h3 className="header-title"> Relationship Types</h3>
                    </div>
                                                        ...
                                                        {showActive ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <button
                                                                    ...
                                                                    title="Delete"
                                                                    onClick={() => deleteFssMastPartyRelationshipTypes(item.id)}
                                                                >
                                                                    <IconTrash />
                                                                </button>
                                                            </>
                                    ...
                                    <div className="row pt-1 mt-1">
                                        ...
                                        <div className="col-md-4 text-left mt-1 ">
                                            ...
                                            <input
                                                ...
                                                defaultValue={editData ? editData.relationship_name : ""} // Set initial value based on editData
                                            />
...
export default FssMastPartyRelationshipTypes;
