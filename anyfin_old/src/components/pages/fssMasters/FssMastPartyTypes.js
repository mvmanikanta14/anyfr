...
const FssMastPartyTypes = () => {
    const [FssMastPartyTypes, setFssMastPartyTypes] = useState([]);
    const [FssMastPartyTypesAdd, setFssMastPartyTypesAdd] = useState([]);
    ...
    const deleteFssMastPartyTypes = (id) => {
        // Construct the correct API URL with the ID
        const deleteUrl = `${apiUrlsService.deleteFssMastPartyTypes}/${id}`;
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    ...
                    getAllFssMastPartyTypes(); // Refresh the list
                } else {
    ...
    const getAllFssMastPartyTypes = () => {
        let pagedata = {
        };

        commonService.add(apiUrlsService.getFssMastPartyTypes, pagedata).then(
            (response) => {
                if (response && response.data) {
                    console.log("mani", response.data)
                    setFssMastPartyTypes(response.data);
    ...
    const onSubmit = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsService.addFssMastPartyTypes, requestData)
                .then((res) => {
                    setFssMastPartyTypesAdd([...FssMastPartyTypesAdd, res.data]);
            ...
            commonService.update(`${apiUrlsService.editFssMastPartyTypes}/${editData.id}`, requestData)
                .then((response) => {
                    if (response) {
                        const updatedFssMastPartyTypes = FssMastPartyTypes.map((item) =>
                            item.id === editData.id ? response.data : item
                        );
                        console.log("Updated Data:", updatedFssMastPartyTypes);
                        setFssMastPartyTypesAdd(updatedFssMastPartyTypes);
    ...
    const handleShowEdit = (id) => {
        const itemToEdit = FssMastPartyTypes.find((item) => item.id === id)
    ...
    const activeShares = FssMastPartyTypes.filter(item => item.is_active);
    const inactiveShares = FssMastPartyTypes.filter(item => !item.is_active);
        ...
        <div className="">
            <div className="">
                <div className="bread_crumb">
                    <div className=" ">
                        <h3 className="header-title"> Party Master</h3>
                    </div>
                                                        ...
                                                        {showActive ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <button
                                                                    ...
                                                                    title="Delete"
                                                                    onClick={() => deleteFssMastPartyTypes(item.id)}
                                                                >
                                                                    <IconTrash />
                                                                </button>
                                                            </>
                                    ...
                                    <div className="row pt-1 mt-1">
                                        ...
                                        <div className="col-md-4 text-left mt-1 ">
                                            ...
                                            <input
                                                ...
                                                {...register("party_type_name", {
                                                    required: true,
                                                })}

                                                defaultValue={editData ? editData.party_type_name : ""} // Set initial value based on editData
                                            />
                                            {errors.party_type_name && (
...
export default FssMastPartyTypes;
