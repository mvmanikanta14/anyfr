...
const FssEntityLineMaster = () => {
    const [FssEntityLineMaster, setFssEntityLineMaster] = useState([]);
    const [FssEntityLineMasterAdd, setFssEntityLineMasterAdd] = useState([]);
    ...
    const deleteFssEntityLineMaster = (id) => {
        // Construct the correct API URL with the ID
        const deleteUrl = `${apiUrlsService.deleteFssEntityLineMaster}/${id}`;
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    ...
                    getAllFssEntityLineMaster(); // Refresh the list
                } else {
    ...
    const getAllFssEntityLineMaster = () => {
        commonService.add(apiUrlsService.getFssEntityLineMaster).then(
            (response) => {
                if (response && response.data) {
                    console.log("mani", response.data)
                    setFssEntityLineMaster(response.data);
        ...
        commonService.add(apiUrlsService.getFssMasterLineMaster).then(
            (response) => {
                if (response && response.data) {
                    ...
                    console.log(' Data:', lineData); // Debugging line
                    setLineMaster(lineData);
                }
            }
        ).catch((error) => {
            handleApiError(error);
    ...
    //     commonService.getAll(apiUrlsService.getFrameworkMaster).then(
    //         (response) => {
    //             if (response && response.data) {
    ...
    const onSubmit = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsService.addFssEntityLineMaster, requestData)
                .then((res) => {
                    setFssEntityLineMasterAdd([...FssEntityLineMasterAdd, res.data]);
            ...
            commonService.update(`${apiUrlsService.editFssEntityLineMaster}/${editData.id}`, requestData)
                .then((response) => {
                    if (response) {
                        const updatedFssEntityLineMaster = FssEntityLineMaster.map((item) =>
                            item.id === editData.id ? response.data : item
                        );
                        console.log("Updated Data:", updatedFssEntityLineMaster);
                        setFssEntityLineMasterAdd(updatedFssEntityLineMaster);
    ...
    const handleShowEdit = (id) => {
        const itemToEdit = FssEntityLineMaster.find((item) => item.id === id)
    ...
    const activeShares = FssEntityLineMaster.filter(item => item.is_active);
    const inactiveShares = FssEntityLineMaster.filter(item => !item.is_active);
            ...
            <div className="">
                <div className="">
                    <div className="bread_crumb">
                        <div className=" ">
                            <h3 className="header-title"> Fss Entity Line Master </h3>
                        </div>

                        <div>
                            <ul>
                                <li className="active"> <Link to={""}> <IconHome /> </Link> </li>
                                <li> Fss Entity Line Master   </li>
                            </ul>
                        </div>
                    </div>
                                                        ...
                                                        {showActive ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <button
                                                                    ...
                                                                    title="Delete"
                                                                    onClick={() => deleteFssEntityLineMaster(item.id)}
                                                                >
                                                                    <IconTrash />
                                                                </button>
                                                            </>
                                    ...
                                    <div className="row pt-1 mt-1">
                                        ...
                                        <div className="col-md-4 text-left mt-1 ">
                                            ...
                                            <input
                                                ...
                                                defaultValue={editData ? editData.custom_name : ""} // Set initial value based on editData
                                            />
                                            {errors.custom_name && (
                                        ...
                                        <div className="col-md-4 text-left mt-1">
                                            <label>Line Master <span className="text-danger">*</span></label>
                                            <Select
                                                options={lineMaster}
                                                onChange={(selectedOption) => setValue("fss_line_master_id", selectedOption.value)}
                                                defaultValue={lineMaster.find(option => option.value === (editData ? editData.fss_line_master_id : ""))}
                                            />
                                        </div>
...
export default FssEntityLineMaster;
