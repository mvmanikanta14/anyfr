...
const FSSCashofAcc = () => {
    const [FSSCashofAcc, setFSSCashofAcc] = useState([]); // Initialize as an empty array
    const [FSSCashofAccUnload, setFSSCashofAccUnload] = useState([]); // Initialize as an empty array
    ...
    const getAllNavigation = () => {
        commonService.getAll(apiUrlsService.getNavigationCOA).then((response) => {
            if (response && response.data) {
    ...
    const getAllPercentage = () => {
        commonService.getAll(apiUrlsService.getPercentage).then((response) => {
            if (response && response.data) {
                setPercentage(response.data.mapping_percentage); // Set the navigation data
            }
        }).catch((error) => {
            handleApiError(error);
        });
    ...
    const getAllEditPopup = () => {
        commonService.getAll(apiUrlsService.getEditFSSCOA).then((response) => {
            if (response && response.data) {
    ...
    const getAllFSSCashofAcc = () => {
        commonService.getAll(apiUrlsService.getFSSCashofAcc).then((response) => {
            if (response && response.data) {
                setFSSCashofAcc(response.data);
            }
        }).catch((error) => {
            handleApiError(error);
        });
    ...
    const getAllFssOptions = () => {
        commonService.getAll(apiUrlsService.getFSSCashofAccActions).then((response) => {
            if (response && response.data) {
                    ...
                    <div className="mb-2">
                        <div>
                            ...
                            <div className="progress-bar-container">
                                ...
                                >
                                    <span className="progress-text">{Percentage ? `${Percentage}% Completed` : 'Loading...'}</span>
                                </div>
                            </div>
                        </div>
                        <div className="table-responsive card">
                            {FSSCashofAcc.length > 0 ? (
                                <table className="table">
                                    <thead>
                                        <tr className="border-btm-0">
                                            ...
                                            <th>GL Group</th>
                                            <th>Mapped to FSS</th>
                                            <th>Considered as Party</th>
                                            <th>Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {FSSCashofAcc.map((item, index) => (
                                            <tr key={item.id}>
                                                <td>{index + 1}</td>
                                                <td>{item.gl_name}</td>
                                                <td>{item.gl_group}</td>
                                                <td>{item.mapped_to_fss || "Not Mapped"}</td>
                                                <td>{item.considered_as_party || "No"}</td>
                                                <td>
