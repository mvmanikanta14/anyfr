...
const ShareParamModeofIssues = () => {
    const [ShareParamModeofIssues, setShareParamModeofIssues] = useState([]);
    const [ShareParamModeofIssuesAdd, setShareParamModeofIssuesAdd] = useState([]);
    ...
    const [searchShareParamModeofIssuesName, setSearchShareParamModeofIssuesName] = useState('');
    ...
    const deleteShareParamModeofIssues = (id) => {
        // Construct the correct API URL with the ID
        const deleteUrl = `${apiUrlsService.deleteShareParamModeofIssues}/${id}`;
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    ...
                    getAllShareParamModeofIssues(); // Refresh the list
                } else {
    ...
    const getAllShareParamModeofIssues = () => {
        let pagedata = {
        };

        commonService.getAll(apiUrlsService.getShareParamModeofIssues, pagedata).then(
            (response) => {
                if (response && response.data) {
                    console.log("mani", response.data)
                    setShareParamModeofIssues(response.data);
    ...
    //     };
    //     if (!ids) {
    //         commonService.add(apiUrlsService.addShareParamModeofIssues, requestData)
    //             .then((res) => {
    //                 setShareParamModeofIssuesAdd([...ShareParamModeofIssuesAdd, res.data])
    ...
    //     else {
    //         data.id = editData.id;
    //         commonService.add(apiUrlsService.editShareParamModeofIssues, data)
    //             .then(
    //                 (response) => {
    //                     if (response) {
    //                         const updatedShareParamModeofIssues = ShareParamModeofIssues.map((item) =>
    //                             item.id === editData.id ? response.data : item
    //                         );
    //                         console.log("manikanta", updatedShareParamModeofIssues)
    //                         setShareParamModeofIssuesAdd(updatedShareParamModeofIssues);
    ...
    const onSubmit = (data) => {
            ...
            // console("Auth is undefined or login_id is missing:", auth);
        ...
        if (!ids) {
            ...
            commonService.update(`${apiUrlsService.editShareParamModeofIssues}/${editData.id}`, requestData)
                .then((response) => {
                    if (response) {
                        const updatedShareParamModeofIssues = ShareParamModeofIssues.map((item) =>
                            item.id === editData.id ? response.data : item
                        );
                        console.log("Updated Data:", updatedShareParamModeofIssues);
                        setShareParamModeofIssuesAdd(updatedShareParamModeofIssues);
    ...
    const handleShowEdit = (id) => {
        const itemToEdit = ShareParamModeofIssues.find((item) => item.id === id)
    ...
    const activeShares = ShareParamModeofIssues.filter(item => item.is_active);
    const inactiveShares = ShareParamModeofIssues.filter(item => !item.is_active);
                                                    ...
                                                    {showActive ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <button
                                                                ...
                                                                title="Delete"
                                                                onClick={() => deleteShareParamModeofIssues(item.id)}
                                                            >
                                                                <IconTrash />
                                                            </button>
                                                        </>
            ...
            <div className="model_box">
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>{isEditMode ? "Edit ShareParamModeofIssues" : "Add ShareParamModeofIssues"}</Modal.Title>
                    </Modal.Header>
                                    ...
                                    <div className="row pt-1 mt-1">
                                        ...
                                        <div className="col-md-4 text-left mt-1 ">
                                            ...
                                            <input
                                                ...
                                                {...register("mode_of_issue_name", {
                                                    required: true,
                                                })}

                                                defaultValue={editData ? editData.mode_of_issue_name : ""} // Set initial value based on editData
                                            />
                                            {errors.mode_of_issue_name && (
...
export default ShareParamModeofIssues;
