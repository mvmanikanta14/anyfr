...
const DebentureReport = () => {
    const [DebentureReport, setDebentureReport] = useState([]);
    const [DebentureReportAdd, setDebentureReportAdd] = useState([]);
    ...
    const deleteDebentureReport = async (id) => {
        ...
        axios
            ...
            .then((response) => {
                ...
                if (Array.isArray(response.data.data) && Array.isArray(response.data.data[0])) {
                    setDebentureReport(response.data); // Extract the first nested array
                    setTotalElements(response.data.totalRecords);
                } else {
                    setDebentureReport(response.data);
                    setTotalElements(response.data.totalRecords);
                }
            })
            .catch((error) => {
    ...
    const handleShowEdit = (id) => {
        const itemToEdit = DebentureReport.find(item => item.sl_no == id)

        // console.log(id, "this is the id for edit data check ")
        // console.log(DebentureReport, "this is the id for edit data hole data")
    ...
    const downloadPDF = async () => {
        ...
        doc.setFontSize(18);
        doc.text('Complete DebentureReport List', 14, 22);
        ...
        doc.autoTable({
            ...
            body: DebentureReport.map((DebentureReport, index

            ) => {
                const sNo = index + 1;
                return [sNo.toString(), DebentureReport.entity_name || ""]
            }),
            theme: 'grid',
            styles: {
                fontSize: 8,
    ...
    const downloadExcel = async () => {
        ...
        //  return false;
        //  }
        const worksheetData = DebentureReport.map((DebentureReport, index) => ({
            'S.No': (pageno - 1) * records + (index + 1),
            'Entitie Name': DebentureReport.entity_name || ""
        }));

        const worksheet = XLSX.utils.json_to_sheet(worksheetData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, 'DebentureReport');
                    ...
                    <div className="">
                        ...
                        <div className="table-responsive">
                            <table className="table table-striped table-bordered table-hover table_custom_1">
                                ...
                                <tbody>
                                    {DebentureReport && DebentureReport.length > 0 ? (
                                        DebentureReport.map((item, index) => {
                                            ...
                                            return (
                                                <tr key={index} onClick={() => handleNoteClick(item.argument, item.type)}>
                                                    ...
                                                    {/* <td className="inline-btns">
                                                        ...
                                                        <button className="btn-danger-outlined" title="Delete" onClick={() => deleteDebentureReport(item.sl_no)}>
                                                            <IconTrash />
                                                        </button>
                                                    </td> */}
                                                </tr>
...
export default DebentureReport;
