...
const FssMasterVoucherTypes = () => {
    const [FssMasterVoucherTypes, setFssMasterVoucherTypes] = useState([]);
    const [FssMasterVoucherTypesAdd, setFssMasterVoucherTypesAdd] = useState([]);
    ...
    const deleteFssMasterVoucherTypes = (id) => {
        // Construct the correct API URL with the ID
        const deleteUrl = `${apiUrlsService.deleteFssMasterVoucherTypes}/${id}`;
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    ...
                    getAllFssMasterVoucherTypes(); // Refresh the list
                } else {
    ...
    const getAllFssMasterVoucherTypes = () => {
        let pagedata = {
        };

        commonService.add(apiUrlsService.getFssMasterVoucherTypes, pagedata).then(
            (response) => {
                if (response && response.data) {
                    console.log("mani", response.data)
                    setFssMasterVoucherTypes(response.data);
    ...
    const onSubmit = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsService.addFssMasterVoucherTypes, requestData)
                .then((res) => {
                    setFssMasterVoucherTypesAdd([...FssMasterVoucherTypesAdd, res.data]);
            ...
            commonService.update(`${apiUrlsService.editFssMasterVoucherTypes}/${editData.id}`, requestData)
                .then((response) => {
                    if (response) {
                        const updatedFssMasterVoucherTypes = FssMasterVoucherTypes.map((item) =>
                            item.id === editData.id ? response.data : item
                        );
                        console.log("Updated Data:", updatedFssMasterVoucherTypes);
                        setFssMasterVoucherTypesAdd(updatedFssMasterVoucherTypes);
    ...
    const handleShowEdit = (id) => {
        const itemToEdit = FssMasterVoucherTypes.find((item) => item.id === id)
    ...
    const activeShares = FssMasterVoucherTypes.filter(item => item.is_active);
    const inactiveShares = FssMasterVoucherTypes.filter(item => !item.is_active);
        ...
        <div className="">
            <div className="">
                <div className="bread_crumb">
                    <div className=" ">
                        <h3 className="header-title"> Voucher Types </h3>
                    </div>
                                                        ...
                                                        {showActive ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <button
                                                                    ...
                                                                    title="Delete"
                                                                    onClick={() => deleteFssMasterVoucherTypes(item.id)}
                                                                >
                                                                    <IconTrash />
                                                                </button>
                                                            </>
                                    ...
                                    <div className="row pt-1 mt-1">
                                        ...
                                        <div className="col-md-4 text-left mt-1 ">
                                            ...
                                            <input
                                                ...
                                                {...register("voucher_type_name", {
                                                    required: true,
                                                })}

                                                defaultValue={editData ? editData.voucher_type_name : ""} // Set initial value based on editData
                                            />
                                            {errors.voucher_type_name && (
...
export default FssMasterVoucherTypes;
