...
const FrameworkMaster = () => {
    const [FrameworkMaster, setFrameworkMaster] = useState([]);
    const [FrameworkMasterAdd, setFrameworkMasterAdd] = useState([]);
    ...
    const activeShares = FrameworkMaster.filter(item => item.is_active);
    const inactiveShares = FrameworkMaster.filter(item => !item.is_active);
    ...
    const deleteFrameworkMaster = (id) => {
        // Construct the correct API URL with the ID
        const deleteUrl = `${apiUrlsService.deleteFrameworkMaster}/${id}`;
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    ...
                    getAllFrameworkMaster(); // Refresh the list
                } else {
    ...
    const getAllFrameworkMaster = () => {
        ...
        commonService.add(apiUrlsService.getFrameworkMaster, pagedata).then(
            (response) => {
                if (response && response.data) {
                    console.log("mani", response.data)
                    setFrameworkMaster(response.data);
    ...
    const onSubmit = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsService.addFrameworkMaster, requestData)
                .then((res) => {
                    setFrameworkMasterAdd([...FrameworkMasterAdd, res.data]);
            ...
            commonService.update(`${apiUrlsService.editFrameworkMaster}/${editData.id}`, requestData)
                .then((response) => {
                    if (response) {
                        const updatedFrameworkMaster = FrameworkMaster.map((item) =>
                            item.id === editData.id ? response.data : item
                        );
                        console.log("Updated Data:", updatedFrameworkMaster);
                        setFrameworkMasterAdd(updatedFrameworkMaster);
    ...
    const handleShowEdit = (id) => {
        const itemToEdit = FrameworkMaster.find((item) => item.id === id)
        ...
        <div className="">
            <div className="">
                <div className="bread_crumb">
                    <div className=" ">
                        <h3 className="header-title"> Framework Master</h3>
                    </div>
                    <div>
                        <ul>
                            <li className="active"> <Link to={""}> <IconHome /> </Link> </li>
                            <li> Framework Master   </li>
                        </ul>
                    </div>
                </div>
                            ...
                            <form className="text-right formtext p-0 mr-0">
                                <div className="row">
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <select
                                            className="Addbutton btn-sm  activehead text-center"
                                                        ...
                                                        {showActive ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <button
                                                                    ...
                                                                    title="Delete"
                                                                    onClick={() => deleteFrameworkMaster(item.id)}
                                                                >
                                                                    <IconTrash />
                                                                </button>
                                                            </>
            ...
            <div className="model_box">
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>{isEditMode ? "Edit Framework" : "Add Framework"}</Modal.Title>
                    </Modal.Header>
...
export default FrameworkMaster;
