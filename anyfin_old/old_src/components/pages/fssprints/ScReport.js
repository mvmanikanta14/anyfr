...
const ScReport = () => {
    ...
    const getAllScReport = () => {
        ...
        axios
            .get(`http://abs.anyfinancials.in:1234/api/screport`, pagedata)
            .then((response) => {
    ...
    const downloadPDF = async () => {
        ...
        doc.setFontSize(18);
        doc.text('Complete ScReport List', 14, 22);
        ...
        doc.autoTable({
            ...
            body: ScReport.map((ScReport, index

            ) => {
                const sNo = index + 1;
                return [sNo.toString(), ScReport.entity_name || ""]
            }),
            theme: 'grid',
            styles: {
                fontSize: 8,
    ...
    const downloadExcel = async () => {
        ...
        //  return false;
        //  }
        const worksheetData = ScReport.map((ScReport, index) => ({
            'S.No': (pageno - 1) * records + (index + 1),
            'Entitie Name': ScReport.entity_name || ""
        }));

        const worksheet = XLSX.utils.json_to_sheet(worksheetData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, 'ScReport');
