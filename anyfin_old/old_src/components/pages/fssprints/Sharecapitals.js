...
const Sharecapitals = () => {
    ...
    const getAllSharecapitals = () => {
        ...
        axios
            .get(`http://demo.anyibc.com/api/apiRoutes/modules/1`, pagedata)
            .then((response) => {
    ...
    const downloadPDF = async () => {
        ...
        doc.setFontSize(18);
        doc.text('Complete Sharecapitals List', 14, 22);
        ...
        doc.autoTable({
            ...
            body: Sharecapitals.map((Sharecapitals, index

            ) => {
                const sNo = index + 1;
                return [sNo.toString(), Sharecapitals.entity_name || ""]
            }),
            theme: 'grid',
            styles: {
                fontSize: 8,
    ...
    const downloadExcel = async () => {
        ...
        //  return false;
        //  }
        const worksheetData = Sharecapitals.map((Sharecapitals, index) => ({
            'S.No': (pageno - 1) * records + (index + 1),
            'Entitie Name': Sharecapitals.entity_name || ""
        }));

        const worksheet = XLSX.utils.json_to_sheet(worksheetData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Sharecapitals');
