...
class GeolocationRaw {
  constructor(subdomain) {
    this.sequelize = getSequelizeInstance(subdomain);
  }

  // Create a new document record
  async save({ latitude,longitude,  user_id}) {
    const query = `
      INSERT INTO geo_locations (latitude, longitude, user_id, created_on)
      VALUES ($1, $2, $3, NOW())
      RETURNING *`;

    const replacements = [latitude,longitude,  user_id];
