const FssParamOepInterUnitRaw = require('../services/dbServices/fssParamOepInterUnitRaw');
...
const createInterUnit = async (req, res) => {
...
if (!result.isValid) {
  return res.status(400).json({
    ...
    const interUnitModel = new FssParamOepInterUnitRaw(subdomain);
  ...
  const editInterUnit = async (req, res) => {
...
await interUnitModel.bulkCreate(
  ...
  const getAllInterUnit = async (req, res) => {
    try {
  ...
  // controller.js
const getInterUnitById = async (req, res) => {
    try {
      const { id } = req.params;
      const subdomain = req.subdomain || "default";
      const organisation_id = req.user.org_id;
  ...
  const deleteInterUnit = async (req, res) => {
