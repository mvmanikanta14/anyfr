...
const getallinvoice = async (req, res) => {
    try {
      ...
      const result = await req.models.ArapArTranInvoicesRaw.getAll(
        page,
        limit,
        entity_id,
        organisation_id,
  ...
  const createInvoice = async (req, res) => {
    try {
      ...
      const resData = await req.models.ArapArTranInvoicesRaw.create(data);
  ...
  const updateInvoice = async (req, res) => {
    try {
      ...
      const updatedData = await req.models.ArapArTranInvoicesRaw.update(data);
  ...
  const deleteInvoice = async (req, res) => {
    try {
      const { id } = req.params;
      const deleted = await req.models.ArapArTranInvoicesRaw.softDelete(id);
