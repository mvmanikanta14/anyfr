...
const getallinvoice = async (req, res) => {
    try {
      ...
      const result = await req.models.ArapAdvrTranInvoicesRaw.getAll(
        page,
        limit,
        entity_id,
        organisation_id,
  ...
  const createInvoice = async (req, res) => {
    try {
      ...
      const resData = await req.models.ArapAdvrTranInvoicesRaw.create(data);
  ...
  const updateInvoice = async (req, res) => {
    try {
      ...
      const updatedData = await req.models.ArapAdvrTranInvoicesRaw.update(data);
  ...
  const deleteInvoice = async (req, res) => {
    try {
      const { id } = req.params;
      const deleted = await req.models.ArapAdvrTranInvoicesRaw.softDelete(id);
