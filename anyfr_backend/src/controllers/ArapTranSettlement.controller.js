...
const getallinvoice = async (req, res) => {
    try {
      ...
      const result = await req.models.ArapTranSettlementRaw.getAll(
        page,
        limit,
        entity_id,
        organisation_id,
  ...
  const getallmodule = async (req, res) => {
    try {
      ...
      const { entity_id , moduleCode} = req.body;
      const organisation_id = req.user.org_id;
  
      const result = await req.models.ArapTranSettlementRaw.getallmodules(
        moduleCode,
        entity_id,
        organisation_id,
  ...
  const createInvoice = async (req, res) => {
    try {
      ...
      const resData = await req.models.ArapTranSettlementRaw.create(data);
  ...
  const updateInvoice = async (req, res) => {
    try {
      ...
      const updatedData = await req.models.ArapTranSettlementRaw.update(data);
  ...
  const deleteInvoice = async (req, res) => {
    try {
      const { id } = req.params;
      const deleted = await req.models.ArapTranSettlementRaw.softDelete(id);
