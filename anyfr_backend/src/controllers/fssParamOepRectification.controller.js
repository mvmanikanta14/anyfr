const fssParamOepRectificationRaw = require('../services/dbServices/fssParamOepRectificationRaw');
...
const createRectification = async (req, res) => {
...
if (!result.isValid) {
  return res.status(400).json({
    ...
    const interUnitModel = new fssParamOepRectificationRaw(subdomain);
  ...
  const editRectification = async (req, res) => {
...
await interUnitModel.bulkCreate(
  ...
  const getAllRectification = async (req, res) => {
    try {
  ...
  // controller.js
const getRectificationById = async (req, res) => {
    try {
      const { id } = req.params;
      const subdomain = req.subdomain || "default";
      const organisation_id = req.user.org_id;
  ...
  const deleteRectification = async (req, res) => {
