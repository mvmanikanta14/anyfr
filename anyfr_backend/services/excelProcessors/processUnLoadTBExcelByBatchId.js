...
const processUnLoadTBExcelByBatchId = async ({organisation_id, entity_id, created_by,period_id,batch_id
     }) => {
 

try {
  //await moduledataTbDbRawService.insertGLsAndValuesFromBatchId( batch_id, entity_id, organisation_id, period_id);
    await moduledataTbDbRawService.unLoadBatch({
  batch_id,
  entity_id,
  organisation_id,
  period_id
  ...
  console.log('Unloaded batch ID:', batch_id || '[no batch]');
} catch (err) {
...
module.exports = { processUnLoadTBExcelByBatchId };
