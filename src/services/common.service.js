...
class CommonService {
  ...
  upload(apiUrl, inputObj) {
  console.log("Sending request to:", apiUrl);
  console.log("Payload:", inputObj);

  // Detect if inputObj is FormData
  const isFormData = (typeof FormData !== 'undefined') && (inputObj instanceof FormData);
  ...
  if (isFormData) {
    ...
    config.headers = {
      'Accept': 'application/json'  // optional, your API response expected type
    };
  } else {
    // For JSON payloads, set Content-Type explicitly (optional, Axios does this by default)
    config.headers = {
  ...
  return api.postForm(apiUrl, inputObj, config)
    .then((response) => {
      console.log("Response received:", response);
      return response;
    })
    .catch((error) => {
      console.error("API Error:", error.response ? error.response.data : error.message);
      return Promise.reject(error.response ? error.response.data : { error: error.message });
    });
}
