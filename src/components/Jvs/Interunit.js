...
const Interunit = () => {
    ...
    const [Interunit, setInterunit] = useState([]); // State to hold chart of accounts data
    ...
    const [InterunitAdd, setInterunitAdd] = useState([]); // State to hold added Interunit
    const [editData, setEditData] = useState(null); // State to hold edit data for Interunit
    const [show, setShow] = useState(false); // State to control modal visibility for Interunit
    ...
    const activeShares = Interunit.filter(item => item.is_active);
    const inactiveShares = Interunit.filter(item => !item.is_active);
    ...
    const debouncedSearch = debounce(() => {
        getAllInterUnitSearch();  // Call the API when searchTerm changes
    }, 500);
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    // Assuming Interunit is the state that holds the list of accounts
                    const updatedInterunit = Interunit.filter((item) => item.id !== id);
                    setInterunit(updatedInterunit); // Update state with the remaining records
    ...
    useEffect(() => {
        if (searchTerm) {
            ...
            setInterunit([]);  // Reset search when searchTerm is empty
        }
            ...
            .then((response) => {
                ...
                if (response && response.data) {
                    if (Array.isArray(response.data)) {
                        setInterunit(response.data); // Set the filtered data
                    } else {
                        swal(response.data.error);
                        setInterunit([]);  // Reset search results if there's an error
                    }
                }
            })
            .catch((error) => {
    ...
    const getAllInterUnit = () => {
        const requestData = {
            entity_id: EntityID,
            period_id: Id,
        };

        commonService.add(apiUrlsService.getAllInterUnit, requestData)
            .then((response) => {
                console.log("API Sai:", response.data);

                if (response && response.data) {
                    if (Array.isArray(response.data.data)) {
                        setInterunit(response.data.data); // Set the filtered data
    ...
    useEffect(() => {
        if (showActive) {
            getAllInterUnit();  // Fetch active entities
        } else {
    ...
    const getAllInactive = () => {
        ...
        commonService.add(apiUrlsService.getAllInactiveInterunit, requestData)
            .then((response) => {
                console.log("API Response:", response.data.data); // Debugging

                if (response && response.data) {
                    if (Array.isArray(response.data.data)) {
                        setInterunit(response.data.data);
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    const deleteUrl = apiUrlsService.deleteInterUnit(ids); // Assuming 'ids' is the ID you want to delete
                    ...
                    commonService.deleteById(deleteUrl)
                        .then((response) => {
                            ...
                            if (response.data && response.data.message) {
                                ...
                                getAllInterUnit(); // Refresh the list or perform any action after deletion
                                swal("Success", "Deleted successfully!", "success");
                                handleClose(); // Close modal or any UI element that needs closing
                            } else {
    ...
    const onSubmit = (data) => {
        ...
        // Extract main row
        /* let total = parseFloat(data.amount || 0);
       
         // Loop through additional fields
         additionalFields.forEach(field => {
           const key = field.id;
           const amountField = `amount_${key}`;
           const value = parseFloat(data[amountField] || 0);
           total += value;
         });
        ...
        additionalFields.forEach((field) => {
            requestData.push({
                ...
                location_id: data[`location_id_${field.id}`],
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsService.addInterunit, requestData)
                .then((res) => {
                    setInterunit([...Interunit, res.data]);
            ...
            // UPDATE Operation
            commonService.update(apiUrlsService.editInterUnit(ids), requestData)
                .then((res) => {
                    const updatedInterunit = Interunit.map((item) =>
                        item.id === ids ? res.data : item
                    );
                    setInterunit(updatedInterunit);
    ...
    const handleShowEditOld = (id) => {
        const itemToEdit = Interunit.find((item) => item.id === id);
    ...
    const handleShowEdit = async (id) => {
        try {
            const res = await commonService.getById(apiUrlsService.getInterUnitById(id));
            //const res = await commonService.getById(apiUrlsService.getInterUnitById(id));
    ...
    const handleClearSearch = () => {
        ...
        setInterunit([]);
        getAllInterUnit();  // Reset the displayed shares to show all data
    };
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="formtext modalform">
                                <div className="container">
                                    ...
                                    <div className="col-lg-12 text-right my-3">
                                        ...
                                        {isEditMode && (
                                            <button type="button" className="btn btn-sm btn-danger" onClick={deleteInterUnit}>
                                                Delete
                                            </button>
                                        )}
                                    </div>
