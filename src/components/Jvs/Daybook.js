...
const Daybook = () => {
    ...
    const [Daybooks, setDaybooks] = useState([]); // State to hold chart of accounts data
    ...
    const [DaybooksAdd, setDaybooksAdd] = useState([]); // State to hold added Daybooks
    const [editData, setEditData] = useState(null); // State to hold edit data for Daybooks
    const [show, setShow] = useState(false); // State to control modal visibility for Daybooks
    ...
    const activeShares = Daybooks.filter(item => item.is_active);
    const inactiveShares = Daybooks.filter(item => !item.is_active);
    ...
    const debouncedSearch = debounce(() => {
        getAllDaybookSearch();  // Call the API when searchTerm changes
    }, 500);
        ...
        commonService.deleteById(deleteUrl)
            .then((response) => {
                ...
                if (response.data.success) {
                    // Assuming Daybooks is the state that holds the list of accounts
                    const updatedDaybooks = Daybooks.filter((item) => item.id !== id);
                    setDaybooks(updatedDaybooks); // Update state with the remaining records
    ...
    useEffect(() => {
        if (searchTerm) {
            ...
            setDaybooks([]);  // Reset search when searchTerm is empty
        }
            ...
            .then((response) => {
                ...
                if (response && response.data) {
                    if (Array.isArray(response.data)) {
                        setDaybooks(response.data); // Set the filtered data
                    } else {
                        swal(response.data.error);
                        setDaybooks([]);  // Reset search results if there's an error
                    }
                }
            })
            .catch((error) => {
    ...
    const getAllDaybook = () => {
        ...
        commonService.add(apiUrlsService.getAllDaybook, requestData)
            .then((response) => {
                console.log("API Sai:", response.data);

                if (response && response.data) {
                    if (Array.isArray(response.data.data)) {
                        setDaybooks(response.data.data); // Set the filtered data
    ...
    useEffect(() => {
        if (showActive) {
            getAllDaybook();  // Fetch active entities
        } else {
    ...
    const getAllInactive = () => {
        ...
        commonService.add(apiUrlsService.getAllInactiveDaybooks, requestData)
            .then((response) => {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    const deleteUrl = apiUrlsService.deleteDaybook(ids); // Assuming 'ids' is the ID you want to delete
                    ...
                    commonService.deleteById(deleteUrl)
                        .then((response) => {
                            ...
                            if (response.data && response.data.message) {
                                ...
                                getAllDaybook(); // Refresh the list or perform any action after deletion
                                swal("Success", "Deleted successfully!", "success");
                                handleClose(); // Close modal or any UI element that needs closing
                            } else {
    ...
    const onSubmit = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsService.addDaybook, requestData)
                .then((res) => {
                    setDaybooks([...Daybooks, res.data]);
            ...
            // UPDATE Operation
            commonService.update(apiUrlsService.editDaybook(ids), requestData)
                .then((res) => {
                    const updatedDaybooks = Daybooks.map((item) =>
                        item.id === ids ? res.data : item
                    );
                    setDaybooks(updatedDaybooks);
    ...
    const handleShowEditOld = (id) => {
        const itemToEdit = Daybooks.find((item) => item.id === id);
    ...
    const handleShowEdit = async (id) => {
        try {
            const res = await commonService.getById(apiUrlsService.getDaybookById(id));
            //const res = await commonService.getById(apiUrlsService.getDaybookById(id));
    ...
    const handleClearSearch = () => {
        ...
        setDaybooks([]);
        getAllDaybook();  // Reset the displayed shares to show all data
    };
                ...
                >
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="formtext modalform">
                        ...
                        <Modal.Footer>
                            ...
                            {isEditMode && (
                                <button type="button" className="btn btn-sm btn-danger" onClick={deleteDaybook}>
                                    Delete
                                </button>
                            )}
                        </Modal.Footer>
