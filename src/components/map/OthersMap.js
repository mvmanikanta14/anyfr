...
const OthersMap = ({ datas }) => {
    ...
    const handleSort = (column) => {
        ...
        getAllOthersMap(); // call API again after sort change
    };
    ...
    const getAllOthersMap = (key = "", value = "") => {
    ...
    const handlePageChange = (newPageNumber) => {
        ...
        commonService.add(`${apiUrlsService.getAllOthresMap}?page=${newPageNumber}&limit=${records}&is_active=${status}`, searchColObj)
            .then((response) => {
                console.log("API:", response.data);
    ...
    const fetchFsliList = () => {
        const requestData = {
            entity_id: EntityID,
            framework_id: tokenService.getFrameworkID()
        };

        commonService
            .add(apiUrlsService.getAllFssFsliMapList, requestData)
            .then((res) => {
                const transformed = (res.data?.data || []).map((item) => ({
                    value: item.id,
                    label: item.flsi_master_name
                }));
                setFsliList(transformed);
            })
            .catch((err) => {
                console.error("FSLI fetch failed:", err);
            });
    };
    ...
    return (
        <div className="">
            ...
            <GLModal
                ...
                OthersMap={OthersMap}
                getAllOthersMap={getAllOthersMap}
