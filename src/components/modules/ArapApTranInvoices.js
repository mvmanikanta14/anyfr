...
const ArapApTranInvoices = ({ datas }) => {
    ...
    const deleteArapAptranInvList = (id) => {
        const deleteUrl = `${apiUrlsModulesService.deleteArapAptranInvList}/${id}`;
    ...
    const getAllListings = (key = "", value = "", passedSortOn = sortOn, passedSortDir = sortDir) => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapAptranInvList}?page=${pageno}&limit=${records}&is_active=${status}`, requestData)
            .then((response) => {
    ...
    const handlePageChange = (newPageNumber) => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapAptranInvList}?page=${newPageNumber}&limit=${records}&is_active=${status}`, searchColObj)
            .then((response) => {
                console.log("API:", response.data);
    ...
    const onSubmitInvRegister = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsModulesService.addArapAptranInvList, requestData)
                .then((res) => {
            ...
            commonService.update(`${apiUrlsModulesService.editArapAptranInvList}`, requestData)
            ...
            <div className="model_box">
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>{isEditMode ? "Account Payable" : "Account Payable"}</Modal.Title>
                    </Modal.Header>
                            ...
                            {showActive ? (
                                <>
                                    {showDeleteButton && (
                                        <button
                                            type="button"
                                            onClick={() => deleteArapAptranInvList(editData?.id)}
...
export default ArapApTranInvoices;
