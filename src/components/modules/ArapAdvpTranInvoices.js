...
const ArapAdvpTranInvoices = ({ datas }) => {
    ...
    const deleteArapAdvptranInvList = (id) => {
        const deleteUrl = `${apiUrlsModulesService.deleteArapAdvptranInvList}/${id}`;
    ...
    const getAllListings = (key = "", value = "", passedSortOn = sortOn, passedSortDir = sortDir) => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapAdvptranInvList}?page=${pageno}&limit=${records}&is_active=${status}`, requestData)
            .then((response) => {
    ...
    const handlePageChange = (newPageNumber) => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapAdvptranInvList}?page=${newPageNumber}&limit=${records}&is_active=${status}`, searchColObj)
            .then((response) => {
                console.log("API:", response.data);
    ...
    const onSubmitInvRegister = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsModulesService.addArapAdvptranInvList, requestData)
                .then((res) => {
            ...
            commonService.update(`${apiUrlsModulesService.editArapAdvptranInvList}`, requestData)
            ...
            <div className="model_box">
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>{isEditMode ? "Advance Payable" : "Advance Payable"}</Modal.Title>
                    </Modal.Header>
                            ...
                            {showActive ? (
                                <>
                                    {showDeleteButton && (
                                        <button
                                            type="button"
                                            onClick={() => deleteArapAdvptranInvList(editData?.id)}
...
export default ArapAdvpTranInvoices;
