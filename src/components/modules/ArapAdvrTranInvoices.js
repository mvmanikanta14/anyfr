...
const ArapAdvrTranInvoices = ({ datas }) => {
    ...
    const deleteArapAdvrtranInvList = (id) => {
        const deleteUrl = `${apiUrlsModulesService.deleteArapAdvrtranInvList}/${id}`;
    ...
    const getAllListings = (key = "", value = "", passedSortOn = sortOn, passedSortDir = sortDir) => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapAdvrtranInvList}?page=${pageno}&limit=${records}&is_active=${status}`, requestData)
            .then((response) => {
    ...
    const handlePageChange = (newPageNumber) => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapAdvrtranInvList}?page=${newPageNumber}&limit=${records}&is_active=${status}`, searchColObj)
            .then((response) => {
                console.log("API:", response.data);
    ...
    const onSubmitInvRegister = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsModulesService.addArapAdvrtranInvList, requestData)
                .then((res) => {
            ...
            commonService.update(`${apiUrlsModulesService.editArapAdvrtranInvList}`, requestData)
            ...
            <div className="model_box">
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>{isEditMode ? "Advance Receivable" : "Advance Receivable"}</Modal.Title>
                    </Modal.Header>
                            ...
                            {showActive ? (
                                <>
                                    {showDeleteButton && (
                                        <button
                                            type="button"
                                            onClick={() => deleteArapAdvrtranInvList(editData?.id)}
...
export default ArapAdvrTranInvoices;
