...
const ArapTranSettlement = ({ datas }) => {
    ...
    const [RefModuledrop, setRefModuledrop] = useState([]);
    ...
    const [moduleCode, setModuleCode] = useState('AR');  // default if you like



    // at top of your component
    const moduleOptions = [
        { value: 'AR', label: 'Account Receivable' },
        { value: 'AP', label: 'Account Payable' },
        { value: 'ADVP', label: 'Advance Payable' },
        { value: 'ADVR', label: 'Advance Receivable' },
    ];
    ...
    }, [moduleCode, EntityID, pageno, records]);
    ...
    const getAllListings = (key = "", value = "", passedSortOn = sortOn, passedSortDir = sortDir) => {
        ...
        const requestData = {
            ...
            value: value,
            searchFilters: {
                settlement_date: key === "settlement_date" ? value.trim() : searchSD.trim(),
                amount_settled: key === "amount_settled" ? value.trim() : searchAS.trim(),
                mode_of_settlement: key === "mode_of_settlement" ? value.trim() : searchMOS.trim(),
                reference_module: key === "reference_module" ? value.trim() :
                    searchRM.trim(),

                party_name: key === "party_name" ? value.trim() :
                    searchPN.trim(),
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapTranSettle}?page=${pageno}&limit=${records}&is_active=${status}`, requestData)
            .then((response) => {
    ...
    const handlePageChange = (newPageNumber) => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapTranSettle}?page=${newPageNumber}&limit=${records}&is_active=${status}`, searchColObj)
            .then((response) => {
                console.log("API:", response.data);
    ...
    const handleModuleChange = (selectedModuleValue) => {
        const requestData = {
            entity_id: EntityID,
            moduleCode: selectedModuleValue,
        };



        // const getAllrefModuledrop = () => {
        ...
        commonService.add(`${apiUrlsModulesService.getAllArapTranSettleDrop}`, requestData)
            .then((response) => {
                ...
                if (response && response.data) {
                    if (Array.isArray(response.data.data)) {
                        ...
                        setRefModuledrop(transformedData);
                    } else {
                        swal(response.data.error);
                        setOthersMap([]);
                    }
    ...
    const onSubmitInvRegister = (data) => {
        ...
        if (!ids) {
            // ADD Operation
            commonService.add(apiUrlsModulesService.addArapTranSettle, requestData)
                .then((res) => {
            ...
            commonService.update(`${apiUrlsModulesService.editArapTranSettle}`, requestData)
                ...
                <table className="table table-bordered table-design-1">
                    <thead>
                        <tr className="bg-light">
                            ...
                            <th onClick={() => handleSort('settlement_date')}>
                                Settlement Date {getSortingIcon('settlement_date')}
                            </th>
                            <th onClick={() => handleSort('amount_settled')}>
                                Amount Settled {getSortingIcon('amount_settled')}
                            </th>
                            <th onClick={() => handleSort('mode_of_settlement')}>
                                Mode of Settlement {getSortingIcon('mode_of_settlement')}
                            </th>
                            <th onClick={() => handleSort('reference_module')}>
                                Reference Module {getSortingIcon('reference_module')}
                            </th>
                            <th onClick={() => handleSort('reference_id')}>
                                Reference Id{getSortingIcon('reference_id')}
                            </th>
                            <th onClick={() => handleSort('reference_number')}>
                                Reference number {getSortingIcon('reference_number')}
                            </th>
                                ...
                                <div className="data-search">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setsearchSD(newvalue);
                                            getAllListings('settlement_date', newvalue);
                                        }}
                                ...
                                <div className="data-search">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            const value = e.target.value;
                                            setsearchAS(value);
                                            getAllListings('amount_settled', value);
                                        }}
                                        className="form-control table-col-search"
                                    />
                                </div>
                                ...
                                <div className="data-search">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            const value = e.target.value;
                                            setsearchMOS(value);
                                            getAllListings('mode_of_settlement', value);
                                        }}
                                        className="form-control table-col-search"
                                    />
                                </div>
                                ...
                                <div className="data-search">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            const value = e.target.value;
                                            setsearchRM(value);
                                            getAllListings('reference_module', value);
                                        }}
                                        className="form-control table-col-search"
                                    />
                                </div>
                            ...
                            displayed.map((item, index) => {
                                ...
                                return (
                                    <tr className="tr-hover-effect1" key={item.id}>
                                        ...
                                        <td onClick={() => handleShowEdit(item.id)}>{item.settlement_date}</td>
                                        <td onClick={() => handleShowEdit(item.id)}>{item.amount_settled}</td>
                                        <td onClick={() => handleShowEdit(item.id)}>{item.mode_of_settlement}</td>
                                        <td onClick={() => handleShowEdit(item.id)}>{item.reference_module}</td>
                                        <td onClick={() => handleShowEdit(item.id)}>{item.invoice_number}</td>
                                        <td onClick={() => handleShowEdit(item.id)}>{item.reference_number}</td>
             ...
             <div className="model_box">
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>{isEditMode ? "Settlement" : "Settlement"}</Modal.Title>
                    </Modal.Header>
                                ...
                                <div className="container">
                                    <div className="row pt-1 mt-1">
                                        ...
                                        <div className="mb-3">
                                            <div className="d-flex align-items-center">
                                                ...
                                                <input
                                                    ...
                                                    {...formInvRegister.register("settlement_date", { required: true })}
                                                    defaultValue={editData ? editData.settlement_date : ""}
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className=" mb-3">
                                            <div className="d-flex align-items-center">
                                                <label className="inline-customform-label me-3 mb-0">
                                                    Amount Settled <span className="text-danger">*</span>
                                                </label>
                                                <input
                                                    type="number"
                                                    placeholder="Enter no of Shares"
                                                    className="form-control"
                                                    {...formInvRegister.register("amount_settled", { required: true })}
                                                    defaultValue={editData ? editData.amount_settled : ""}
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className=" mb-3">
                                            <div className="d-flex align-items-center">
                                                <label className="inline-customform-label me-3 mb-0">
                                                    Mode of Settlement <span className="text-danger">*</span>
                                                </label>
                                                <input
                                                    type="text"
                                                    placeholder="Enter no of Shares"
                                                    className="form-control"
                                                    {...formInvRegister.register("mode_of_settlement", { required: true })}
                                                    defaultValue={editData ? editData.mode_of_settlement : ""}
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="mb-3">
                                            <div className="d-flex align-items-center">
                                                <label className="inline-customform-label me-3 mb-0">
                                                    Reference Module <span className="text-danger">*</span>
                                                </label>
                                                <div className="flex-grow-1">
                                                    <Controller
                                                        name="reference_module"
                                                        control={formInvRegister.control}
                                                        defaultValue={editData ? editData.reference_module : ""}
                                                        rules={{ required: true }}
                                                        render={({ field }) => (
                                                            <Select
                                                                {...field}
                                                                ...
                                                                value={moduleOptions.find(opt => opt.value === field.value) || null}
                                                                onChange={(opt) => {
                                                                    field.onChange(opt.value);
                                                                    handleModuleChange(opt.value); // Call API based on selection
                                                                }}
                                        ...
                                        <div className=" mb-3">
                                            <div className="d-flex align-items-center">
                                                ...
                                                <div className="flex-grow-1">
                                                    <Controller
                                                        key={moduleCode}                   // ← rebuilds on each module change
                                                        name="reference_id"
                                                        control={formInvRegister.control}
                                                        defaultValue={editData ? editData.reference_id : ''}
                                                        rules={{ required: true }}
                                                        render={({ field }) => (
                                                            <Select
                                                                {...field}
                                                                options={RefModuledrop}
                                                                isSearchable={true}
                                                                placeholder="Select"
                                                                classNamePrefix="custom-select"
                                                                onChange={(selectedOption) => field.onChange(selectedOption?.value)}
                                                                value={RefModuledrop.find(option => option.value === field.value)}
                                                                menuPlacement="auto"
                                                                menuPosition="fixed"
                                                            />
                                                        )}
...
export default ArapTranSettlement;
